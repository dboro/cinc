  ...
  <div>
    ...
    >
      <template v-slot:item.id="{ item }">
        <v-btn @click.stop="$router.push({name: 'admin-products-edit-id', params: {id: item.id}})" class="ma-2" outlined x-small fab color="indigo">
...
export default {
  ...
  methods: {
    ...
    load() {
      this.loading = true
      this.$store.dispatch('admin/products/getAll', {
        success: (data) => this.items = data,
        end: () => this.loading = false
      })
    },
    destroy() {
      this.$store.dispatch('admin/products/destroy', {
